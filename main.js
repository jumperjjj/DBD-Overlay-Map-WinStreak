...
function makeWindows(){
 ...
 for(const w of [streak,mapWin]){
  ...
  const constrain=()=>{if(fixing)return;fixing=true;const raw=w.getBounds(),c=clampRect(raw);if(raw.x!==c.x||raw.y!==c.y||raw.width!==c.width||raw.height!==c.height)w.setBounds(c);last=c;dirty(true);fixing=false};
...
function edit(on){
 for(const [w,k] of [[streak,'streak'],[mapWin,'map']]){
