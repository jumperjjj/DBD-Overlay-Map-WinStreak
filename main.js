...
const defaults={
 ...
 teamEnabled:false,teamA:'TIME A',teamB:'TIME B',teamScoreA:0,teamScoreB:0,teamColorA:'#a86cff',teamColorB:'#6cff8d',
 matchStyle:0,matchSetText:'SET 1/1',matchBg:'#14171b',matchPanel:'#25292d',matchText:'#ffffff',matchMuted:'#c8c8c8',
 matchPadding:12,matchHeaderHeight:66,matchRowHeight:28,matchGap:4,matchFooterHeight:24,matchTeamFontSize:22,matchScoreFontSize:32,
 matchFooterShow:true,matchFooter:'SET 1: THE COAL TOWER',
 matchRows:[
  {show:true,label:'SURVIVOR RESULT',a:'7 STAGES - 3F',b:'6 STAGES - 2F'},
  {show:true,label:'KILLER WINCON',a:'6 STAGES',b:'7 STAGES'},
  {show:false,label:'INFO 3',a:'',b:''},
  {show:false,label:'INFO 4',a:'',b:''}
 ],
 team:{x:500,y:55,w:820,h:210,visible:true,scale:1}
}
let S;
const sp=()=>path.join(app.getPath('userData'),'settings.json');
function load(){try{const j=JSON.parse(fs.readFileSync(sp(),'utf8'));S={...defaults,...j,streak:{...defaults.streak,...(j.streak||{})},team:{...defaults.team,...(j.team||{})},matchRows:Array.isArray(j.matchRows)?j.matchRows:structuredClone(defaults.matchRows)}}catch{S=structuredClone(defaults)}
 S.streak.scale=Math.max(.78,Math.min(1.22,+S.streak.scale||1));S.team.scale=Math.max(.70,Math.min(1.18,+S.team.scale||1));
 S.streak.w=Math.round(380*S.streak.scale);S.streak.h=Math.round(120*S.streak.scale);S.team.w=Math.round(820*S.team.scale);S.team.h=Math.round(210*S.team.scale);
}
...
ipcMain.handle('patch',(_,p)=>{S={...S,...p};if(p.streak)S.streak={...S.streak,...p.streak};if(p.team)S.team={...S.team,...p.team};if(p.matchRows)S.matchRows=p.matchRows.map((r,i)=>({...defaults.matchRows[i],...r}));S.nameFontSize=Math.max(10,Math.min(42,+S.nameFontSize||18));S.numberFontSize=Math.max(24,Math.min(100,+S.numberFontSize||58));S.bgOpacity=Math.max(0,Math.min(1,Number(S.bgOpacity??1)));save();if(!Object.keys(p).every(k=>['language','hotkey'].includes(k)))dirty(true);return S});
